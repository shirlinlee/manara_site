        ...
        <div class="mycondition_area clearfix">
            ...
            <form action="order.php" method="post">
                ...
                <p class="btn">
                    ...
                    <input type="image" onmouseover="chgImg('<!--{$TPL_URLPATH}-->img/button/btn_order_re_on.jpg', this);" onmouseout="chgImg('<!--{$TPL_URLPATH}-->img/button/btn_order_re.jpg', this);" src="<!--{$TPL_URLPATH}-->img/button/btn_order_re.jpg" name="submit" value="この購入内容で再注文する" />
<!--{*                    <input type="hidden" name="order_id" value="<!--{$tpl_arrOrderData.order_id|h}-->">
					<button class="bt01">再訂購此次購買內容</button> *}-->
                </p>
            </form>
        </div>
                ...
                <tr>
                    ...
                    <!--{if $orderDetail.product_type_id == $smarty.const.PRODUCT_TYPE_DOWNLOAD}-->
                        <!--{if $orderDetail.is_downloadable}-->
                            <a target="_self" href="<!--{$smarty.const.ROOT_URLPATH}-->mypage/download.php?order_id=<!--{$tpl_arrOrderData.order_id}-->&product_id=<!--{$orderDetail.product_id}-->&product_class_id=<!--{$orderDetail.product_class_id}-->">下載</a>
                        <!--{else}-->
                            <!--{if $orderDetail.payment_date == "" && $orderDetail.effective == "0"}-->
                                <!--{$arrProductType[$orderDetail.product_type_id]}--><BR />(付款確認中)
                            <!--{else}-->
                                <!--{$arrProductType[$orderDetail.product_type_id]}--><BR />(已過期)
                            <!--{/if}-->
                        <!--{/if}-->
                    <!--{else}-->
                    ...
                    <td class="alignR"><!--{$price|sfCalcIncTax|number_format|h}-->新台幣</td>
                    <td class="alignR"><!--{$quantity|h}--></td>
                    <td class="alignR"><!--{$price|sfCalcIncTax|sfMultiply:$quantity|number_format}-->新台幣</td>
                </tr>
            <!--{/foreach}-->
            <tr>
                <th colspan="5" class="alignR">小計</th>
                <td class="alignR"><!--{$tpl_arrOrderData.subtotal|number_format}-->新台幣</td>
            </tr>
            <!--{assign var=point_discount value="`$tpl_arrOrderData.use_point*$smarty.const.POINT_VALUE`"}-->
            <!--{if $point_discount > 0}-->
            <tr>
                <th colspan="5" class="alignR">紅利點數折扣</th>
                <td class="alignR">&minus;<!--{$point_discount|number_format}-->新台幣</td>
            </tr>
            <!--{/if}-->
            <!--{assign var=key value="discount"}-->
            <!--{if $tpl_arrOrderData[$key] != "" && $tpl_arrOrderData[$key] > 0}-->
            <tr>
                <th colspan="5" class="alignR">折扣</th>
                <td class="alignR">&minus;<!--{$tpl_arrOrderData[$key]|number_format}-->新台幣</td>
            </tr>
            <!--{/if}-->
            <tr>
                <th colspan="5" class="alignR">運費</th>
                <td class="alignR"><!--{assign var=key value="deliv_fee"}--><!--{$tpl_arrOrderData[$key]|number_format|h}-->新台幣</td>
            </tr>
            <tr>
                <th colspan="5" class="alignR">手續費</th>
                <!--{assign var=key value="charge"}-->
                <td class="alignR"><!--{$tpl_arrOrderData[$key]|number_format|h}-->新台幣</td>
            </tr>
            <tr>
                <th colspan="5" class="alignR">合計</th>
                <td class="alignR"><span class="price"><!--{$tpl_arrOrderData.payment_total|number_format}-->新台幣</span></td>
            </tr>
        </table>
        ...
        <!--{foreach item=shippingItem name=shippingItem from=$arrShipping}-->
                ...
                <table summary="お届け内容確認/">
                    ...
                    <!--{foreach item=item from=$shippingItem.shipment_item}-->
                        <tr>
                            ...
                            <!--{* XXX 購入小計と誤差が出るためコメントアウト
                            <td class="alignR"><!--{$item.total_inctax|number_format}--></td>新台幣
                            *}-->
                        </tr>
                    <!--{/foreach}-->
                </table>
            ...
            <table summary="送貨地址" class="delivname">
                <tbody>
                    ...
                    <tr>
                        <th class="alignL">到貨日</th>
                        <td><!--{$shippingItem.shipping_date|default:'未選定'|h}--></td>
                    </tr>
                    <tr>
                        <th class="alignL">到貨時間</th>
                        <td><!--{$shippingItem.shipping_time|default:'未選定'|h}--></td>
                    </tr>
                </tbody>
            </table>
        <!--{/foreach}-->
